...
export default {
  ...
  mounted () {
    ...
    hotkeys.filter = () => true
    hotkeys('tab,ctrl+2,command+2,ctrl+3,command+3,ctrl+b,command+b,ctrl+k,command+k,ctrl+u,command+u,ctrl+i,command+i,ctrl+shift+i,command+shift+i,ctrl+z,command+z,ctrl+shift+z,command+shift+z', (event, handler) => {
      event.preventDefault()
      let template = ''
      let bothEnds = ''
      switch (handler.key) {
        case 'tab':template=`	`;break;
        case 'ctrl+2': // 二级标题
        case 'command+2':
          template = `
## text  `
